...
def home_axis(step_pin, dir_pin, endstop_pin):
    ...
    GPIO.output(dir_pin, GPIO.LOW)  # Move towards endstop
    while not check_endstop(endstop_pin):
        GPIO.output(step_pin, GPIO.HIGH)
        sleep(0.005)  # Adjust speed as needed
        GPIO.output(step_pin, GPIO.LOW)
        sleep(0.005)
...
def move_to_position(step_pin, dir_pin, current_position, target_position, speed):
    ...
    move_motor(step_pin, dir_pin, steps_to_move, direction, speed)
...
class MotorControlApp:
    def __init__(self, root):
        self.root = root
        self.root.title("3D Scanner Motor Control")
        ...
        self.control_frame.grid(row=0, column=1, padx=5, pady=5)
        ...
        self.x_slider.grid(row=0, column=0, columnspan=2)
        ...
        self.y_slider.grid(row=1, column=0, columnspan=2)
        ...
        self.home_button.grid(row=7, column=0)

        self.run_button = tk.Button(self.control_frame, text="Run Procedure", command=self.run_procedure)
    ...
    def move_x_to_minus180(self):
        ...
        current_position_x = move_to_position(STEP_PIN_X, DIR_PIN_X, current_position_x, -180, 0.001)
        self.x_slider.set(current_position_x)

    def move_x_to_zero(self):
        """Move X motor to 0."""
        global current_position_x
        current_position_x = move_to_position(STEP_PIN_X, DIR_PIN_X, current_position_x, 0, 0.001)
        self.x_slider.set(current_position_x)
    ...
    def move_y_to_minus180(self):
        ...
        current_position_y = move_to_position(STEP_PIN_Y, DIR_PIN_Y, current_position_y, -180, 0.001)
        self.y_slider.set(current_position_y)

    def move_y_to_zero(self):
        """Move Y motor to 0."""
        global current_position_y
        current_position_y = move_to_position(STEP_PIN_Y, DIR_PIN_Y, current_position_y, 0, 0.001)
        self.y_slider.set(current_position_y)
    ...
    def move_z_to_minus180(self):
        ...
        current_position_z = move_to_position(STEP_PIN_Z, DIR_PIN_Z, current_position_z, -180, 0.001)
        self.z_slider.set(current_position_z)

    def move_z_to_zero(self):
        """Move Z motor to 0."""
        global current_position_z
        current_position_z = move_to_position(STEP_PIN_Z, DIR_PIN_Z, current_position_z, 0, 0.001)
        self.z_slider.set(current_position_z)
    ...
    def move_x_left(self):
        ...
        current_position_x = move_by_steps(STEP_PIN_X, DIR_PIN_X, current_position_x, step_count, GPIO.LOW, 0.001)
        self.x_slider.set(current_position_x)
    ...
    def move_x_right(self):
        ...
        global current_position_x
        step_count = int(self.x_step_entry.get())
        current_position_x = move_by_steps(STEP_PIN_X, DIR_PIN_X, current_position_x, step_count, GPIO.HIGH, 0.001)
        self.x_slider.set(current_position_x)
    ...
    def move_y_left(self):
        ...
        current_position_y = move_by_steps(STEP_PIN_Y, DIR_PIN_Y, current_position_y, step_count, GPIO.LOW, 0.001)
        self.y_slider.set(current_position_y)
    ...
    def move_y_right(self):
        ...
        global current_position_y
        step_count = int(self.y_step_entry.get())
        current_position_y = move_by_steps(STEP_PIN_Y, DIR_PIN_Y, current_position_y, step_count, GPIO.HIGH, 0.001)
        self.y_slider.set(current_position_y)
    ...
    def move_z_left(self):
        ...
        current_position_z = move_by_steps(STEP_PIN_Z, DIR_PIN_Z, current_position_z, step_count, GPIO.LOW, 0.001)
        self.z_slider.set(current_position_z)
    ...
    def move_z_right(self):
        ...
        global current_position_z
        step_count = int(self.z_step_entry.get())
        current_position_z = move_by_steps(STEP_PIN_Z, DIR_PIN_Z, current_position_z, step_count, GPIO.HIGH, 0.001)
        self.z_slider.set(current_position_z)
    ...
    def x_slider_moved(self, value):
        ...
        current_position_x = move_to_position(STEP_PIN_X, DIR_PIN_X, current_position_x, target_position_x, 0.001)

    def y_slider_moved(self, value):
        """Callback for when the Y-axis slider is moved."""
        global current_position_y
        target_position_y = int(value)
        current_position_y = move_to_position(STEP_PIN_Y, DIR_PIN_Y, current_position_y, target_position_y, 0.001)

    def z_slider_moved(self, value):
        """Callback for when the Z-axis slider is moved."""
        global current_position_z
        target_position_z = int(value)
        current_position_z = move_to_position(STEP_PIN_Z, DIR_PIN_Z, current_position_z, target_position_z, 0.001)
    ...
    def run_procedure(self):
        ...
        run_procedure(x_steps, y_steps, z_steps, focus_intervals, 0.001)
